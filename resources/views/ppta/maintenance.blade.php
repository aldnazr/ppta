...
@section('content')
    <div class="container bg-white border border-gray-200 rounded-xl shadow-lg p-6">
        <div class="flex flex-col mb-6 space-y-2">
            <p class="text-2xl font-bold justify-center lg:justify-start">Maintenance Data Penguji</p>
            <div
                ...
                <!-- Filter Form -->
                <form method="GET" action="{{ url()->current() }}" class="relative" x-data="{ isOpen: false }">
                    <button type="button" @click="isOpen = !isOpen" data-filter-toggle
                    ...
                    <div id="advancedFilterDropdown" x-show="isOpen" @click.outside = "isOpen = false"
                        ...
                        <div class="space-y-2 text-sm">
                            <select name="tingkat"
                                ...
                                @foreach ($uniqueTingkat as $item)
                                    <option value="{{ $item }}" {{ $selectedTingkat == $item ? 'selected' : '' }}>
                                        {{ $item }}</option>
                                @endforeach
                            </select>
                            <select name="status"
